...
def optimizer_step(_loss, _optimizer, _model, _schedule, args, mask, is_update = True):
  if args.fp16:
...
def train_validate(model, optimizer, scheduler, train_loader, valid_loader, args, mask, train_step = 0, epoch = 0):
  model.train()
  avg_lm_loss = AverageMeter()
  ...
  for idx, data in enumerate(train_loader):
    ...
    optimizer_step(_lm_loss/(args.grad_acc), optimizer, model, scheduler, args, mask, is_update=is_update)
...
if __name__ == '__main__':
  ...
  try:
    ...
    for epoch in itertools.count(start=1):
      ...
      train_step = train_validate(lm_net, optimizer, scheduler, train_loader, valid_loader, args, mask, train_step=train_step, epoch = epoch)
