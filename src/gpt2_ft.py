...
if __name__ == '__main__':
	...
	if args.lora_dim == 0:
		...
		optimizer_grouped_parameters = [
				{
						"params": [p for n, p in lm_net.named_parameters() if 'adapter' in n or 'S_' in n],
				}
		]
	...
	for name, module in lm_net.named_modules():
		if isinstance(module, Attention):
			module.S_Q.data = torch.zeros(1024, 1024).to(module.S_Q.weight.device)
			module.S_V.data = torch.zeros(1024, 1024).to(module.S_Q.weight.device)
	...
	for name, module in lm_net.named_modules():
		...
		if isinstance(module, Attention):
			...
			# print((S_Q!=0).sum())
			# print(S_Q.sum(1))
			prune.custom_from_mask(module.S_Q, 'weight', S_Q)
			prune.custom_from_mask(module.S_V, 'weight', S_V)
