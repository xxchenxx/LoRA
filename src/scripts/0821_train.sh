nohup python -m torch.distributed.launch --nproc_per_node=4 --master_port=5678 src/gpt2_ft.py \
    ...
    --random_seed 110 \
    --save_interval 10000 > 0822_train_lora_2.out &


CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.launch --nproc_per_node=2 --master_port=1234 src/gpt2_beam.py \
    --data ./data/dart/test.jsonl \
    --batch_size 1 \
    --seq_len 512 \
    --eval_len 64 \
    --model_card gpt2.md \
    --init_checkpoint ./trained_models/GPT2_M/dart/model.39160.pt \
    --platform local \
    --lora_dim 3 \
    --lora_alpha 32 \
    --beam 10 \
...
python src/gpt2_decode.py \
        --vocab ./vocab \
        --sample_file ./trained_models/GPT2_M/dart/predict.39160.b10p08.jsonl \
